...
class ASEUDR:
    ...
    def initialize_argparse(self):
        ...
        parser.add_argument('-brd', '--baseline_root_domain',
                            help='Specify the baseline root domain (non-geolocated) to validate the DNS resolvers ('
                                 'default: bbc.com).',
                            default='bbc.com')
        parser.add_argument('-nxd', '--nxdomain',
                            help='Specify the root domain to validate the DNS resolvers using the NXDOMAIN validation '
                                 '(default: google.com).',
                            default='google.com')
        ...
        parser.add_argument('-m', '--mode',
                            help='Specify the desired mode (default: resolver).',
                            default='resolver',
        ...
        if args.disable_nxdomain_validation and (args.nxdomain != 'google.com'):
            self.console.print('\n[bold red][-] --nxdomain can not be used with --disable_nxdomain_validation.[/bold '
                               'red]')
            exit()

        if args.mode == 'brute-force' and (not args.wordlist_file_path or not args.domain):
            self.console.print('\n[bold red][-] brute-force mode can not be used without --wordlist_file_path and '
                               '--domain.[/bold red]')
            exit()

        if args.mode == 'resolver' and (args.wordlist_file_path or args.domain):
            self.console.print('\n[bold red][-] --wordlist_file_path or --domain can be used with brute-force mode.['
                               '/bold red]')
            exit()

        if args.mode == 'resolver' and not args.domains_file_path:
            self.console.print('\n[bold red][-] --mode resolver and --domains_file_path should be used together.['
                               '/bold red]')
            exit()
    ...
    def is_baseline_root_domain_non_geolocated(self):
        ...
        if self.args.baseline_root_domain == 'bbc.com':
            ...
            self.console.print('\n[bold yellow][*] Starting verification: checking if the baseline root domain is '
                               'non-geolocated.[/bold yellow]')
    ...
    def same_ip_address_validator(self):
        self.console.print('\n[bold yellow][*] Starting verification: checking if the DNS resolver and trusted DNS '
                           'resolver return the same IP address.[/bold yellow]')
    ...
    def nxdomain_validator(self):
        self.console.print('\n[bold yellow][*] Starting verification: checking if the DNS resolver returns NXDOMAIN '
                           'for non-existent subdomains of known target root domains.[/bold yellow]')
